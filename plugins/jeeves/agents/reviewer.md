...
the user's call: you post only when the loop messages you their pick.
...
status: "done"|"blocked"|"error", summary, verdict? }` — summary carries the report, verdict is
APPROVE / UNAPPROVE when vetting — do not merely print it. Otherwise, return the report as text.
...
rather than submitting over it. Then report again with
the posted review's URL in `summary`.
