...
| `write_state` | `{ project, markdown }` writes `projects/<project>/state.md`; `{ file: 'reminders', markdown }` writes `reminders.md`; `{ file: 'daily', markdown }` writes `daily.md`. `markdown` is the whole file; `edits: [{ old, new }]` in its place changes rows in place, each `old` matching exactly once. |
...
| `open_space` | `{ repo?, branch? \| pr? \| path?, tab?, command? \| prompt?, label? }` — opens a space in the browser for the user (a branch's worktree, reused or created; a PR's head branch; an existing worktree; or the main checkout) and returns a `spaceRef` and its first tab's `tabRef`. `prompt` starts the first claude tab on that message. `command` opens a shell tab that runs it once the shell first prints (on that tab's first spawn only). With no `repo`, `path` alone opens a **folder space**: any existing folder inside the allowed roots (`GET /api/folder?path=` resolves `~` and checks it), with no repo, worktree or dashboard rows. Refuses when no browser tab is connected. |
